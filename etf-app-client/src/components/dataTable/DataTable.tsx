...
const SortableTh: React.FC<SortableThProps> = ({
  ...
  return (
    <th onClick={onClick} className="table-head">
      {label} {active ? (direction === SortDirection.Asc ? "↑" : "↓") : ""}
    </th>
  );
};

const DataTable: React.FC<DataTableProps> = ({ tableData }) => {
  const [sortKey, setSortKey] = useState<SortKey>(SortKey.Constituent);
  ...
  const handleSort = (nextKey: SortKey) => {
    ...
    setSortKey(nextKey);
    setSortDirection(
      nextKey === SortKey.Constituent ? SortDirection.Asc : SortDirection.Desc,
    );
  };
          ...
          <thead>
            <tr>
              ...
              <SortableTh
                ...
                onClick={() => handleSort(SortKey.Constituent)}
              />
              <SortableTh
                label="Weight"
              ...
              <SortableTh
                ...
                direction={sortDirection}
                onClick={() => handleSort(SortKey.LatestClose)}
              />
            </tr>
          </thead>
          ...
          <tbody>
            ...
            {sortedRows.map((row, index) => (
              <tr key={`${row.name}-${index}`}>
                ...
                <td className="table-cell">{formatWeight(row.weight)}</td>
                <td className="table-cell">{`$${row.latestClosePrice}`}</td>
              </tr>
            ))}
          </tbody>
        </table>
