...
const HoldingsBarChart: React.FC<HoldingsBarChartProps> = ({ data, title }) => {
  // For each constituent, multiply the
  // filter out the holdings with invalid value and sort to obtain the biggest 5
  const barChartData = data
    .map((item) => {
      if (
        item.latestClosePrice === undefined ||
        Number.isNaN(item.latestClosePrice)
      ) {
        return null;
      }
  ...
  return (
    <div>
      ...
      >
        ...
        <Series
          type="column"
          data={barChartData.map((item) => Number(item.value.toFixed(2)))}
        />
      </Chart>
    </div>
  );
